...
export async function saveProgress(tasks: any, totalXP: number) {
  try {
    ...
    // السطر السحري اللي بيعمل تحديث فوري للوحة المتصدرين
    revalidatePath("/");
  } catch (e) {
    console.log(e);
  }
...
2. تحديث ملف المهام (عشان يبعت النقط صح)

  - افتحي ملف tasks-card.tsx جوه مجلد components/dashboard.
  - امسحي كل اللي فيه، وانسخي الكود ده (ظبطتلك فيه ألوان المهام بالأخضر عشان
    تليق مع الموقع 😍)، ودوسي Commit:
