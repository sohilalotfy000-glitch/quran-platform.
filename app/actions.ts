...
export async function saveProgress(tasks: any, totalXP: number, uId: string, uName: string, uPic: string) {
  if (!uId) return;
  try {
    // غيرنا اسم الخزنة لـ batch2 عشان يفتح صفحة جديدة للدفعة الجديدة
    let board: any = (await kv.get("quran_board_batch2")) || [];
    if (!Array.isArray(board)) board = [];
    
    board = board.filter((user: any) => user.id !== uId);
    board.push({ id: uId, name: uName || "طالب", avatar: uPic || "", xp: totalXP });
    
    await kv.set("quran_board_batch2", board);
    revalidatePath("/");
  } catch (e) {
...
3.  دوسي Commit changes.

2. تصفير المهام وعلامات الصح (ملف المهام):

1.  روحي لملف tasks-card.tsx جوه مجلد components/dashboard.
2.  امسحي كل اللي فيه، وانسخي الكود ده وحطيه:
...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  useEffect(() => {
    setMounted(true);
    // غيرنا الكاش للدفعة الثانية عشان يمسح علامات الصح القديمة للكل
