...
const lectureTasksData =[
  ...
  // المحاضرة الثامنة (المهام الجديدة)
...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  useEffect(() => {
    setMounted(true);
    // حدثنا اسم الذاكرة لـ v15 عشان الكل يشوف الجديد فوراً
    const saved = localStorage.getItem("quran-tasks-final-v15");
  ...
  const toggleTask = async (taskId: string) => {
    ...
    setTasks(newTasks);
    localStorage.setItem("quran-tasks-final-v15", JSON.stringify(newTasks));
