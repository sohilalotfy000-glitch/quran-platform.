...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  // دلوقتي المهام بقت محفوظة باسم المتدرب، مش بتتلخبط لو فتحتي 100 إيميل
  useEffect(() => {
    setMounted(true);
    if (isSignedIn && user) {
      const saved = localStorage.getItem(`quran-tasks-new-${user.id}`);
      if (saved) setTasks(JSON.parse(saved));
      else setTasks(lectureTasksData); // لو متدرب جديد بيبدأ من الصفر
    }
  }, [isSignedIn, user]);
  ...
  const toggleTask = async (taskId: string) => {
    if (!isSignedIn || !user) return; // لازم يسجل عشان يحفظ
    
    const newTasks = tasks.map((t: any) => t.id === taskId ? { ...t, completed: !t.completed } : t);
    setTasks(newTasks);
    
    localStorage.setItem(`quran-tasks-new-${user.id}`, JSON.stringify(newTasks));
