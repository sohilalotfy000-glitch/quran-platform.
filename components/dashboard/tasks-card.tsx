...
const lectureTasksData: Task[] =[
  ...
  { id: "l2_1", lectureGroup: "المحاضرة الثانية", title: "تحديد الهدف", description: "الهدف", xp: 5, completed: false },
...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem("quran-tasks-final-1");
  ...
  const toggleTask = async (taskId: string) => {
    ...
    setTasks(newTasks);
    localStorage.setItem("quran-tasks-final-1", JSON.stringify(newTasks));
