...
const lectureTasksData =[
  { id: "l1_1", lectureGroup: "المحاضرة الأولى", title: "مراجعة 18 سورة", description: "تدريب ومراجعة", xp: 5, completed: false },
  { id: "l1_2", lectureGroup: "المحاضرة الأولى", title: "تذكر الصور", description: "ميموري ليج", xp: 5, completed: false },
  { id: "l1_3", lectureGroup: "المحاضرة الأولى", title: "تدبر 5 آيات", description: "تدبر", xp: 5, completed: false },
  { id: "l2_1", lectureGroup: "المحاضرة الثانية", title: "تحديد الهدف", description: "تحديد الهدف", xp: 5, completed: false },
  { id: "l2_2", lectureGroup: "المحاضرة الثانية", title: "الاستدعاء النشط", description: "التدرب", xp: 5, completed: false },
  ...
  { id: "l3_1", lectureGroup: "المحاضرة الثالثة", title: "حفظ في نصف ساعة", description: "حفظ", xp: 5, completed: false },
  { id: "l3_2", lectureGroup: "المحاضرة الثالثة", title: "تهيئة وسماع", description: "7 دقائق", xp: 5, completed: false },
  { id: "l3_3", lectureGroup: "المحاضرة الثالثة", title: "تمارين التركيز", description: "بومودورو", xp: 5, completed: false },
  ...
  { id: "l4_5", lectureGroup: "المحاضرة الرابعة", title: "حفظ صفحة", description: "20 دقيقة", xp: 5, completed: false }
];
...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem("quran-tasks-final-v6");
  ...
  const toggleTask = async (taskId: string) => {
    ...
    setTasks(newTasks);
    localStorage.setItem("quran-tasks-final-v6", JSON.stringify(newTasks));
    
    // إرسال البطاقة مع النقط
    if (isSignedIn && user) {
