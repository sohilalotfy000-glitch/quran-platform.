...
export function TasksCard({ type }: { type: "daily" | "lecture" }) {
  ...
  useEffect(() => {
    setMounted(true);
    // كود جديد للدفعة دي عشان تمسح الكاش
    const saved = localStorage.getItem("quran-tasks-real-v1");
  ...
  const toggleTask = async (taskId: string) => {
    ...
    setTasks(newTasks);
    localStorage.setItem("quran-tasks-real-v1", JSON.stringify(newTasks));
    ...
    if (isSignedIn && user) {
      ...
      router.refresh(); // ده بيعمل تحديث فوري للشاشة عشان اسمك يظهر في ثانية
    }
  };
